    ...
    <div className="max-w-3xl mx-auto p-6 mt-10 bg-white shadow-lg rounded-xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-4">
          <label className="block text-sm font-medium text-gray-700">이미지 첨부 ({selectedFiles.length}개)</label>
            ...
            <div className="flex flex-wrap gap-3 p-3 bg-gray-50 rounded-lg border">
              {previews.map((url, index) => (
                <div key={index} className="relative w-20 h-20 group">
                  <img src={url} className="w-full h-full object-cover rounded-md border" alt="미리보기" />
                  <button
                    type="button"
          ...
          <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 transition-colors">
            <div className="flex flex-col items-center justify-center pt-5 pb-6">
              <p className="mb-2 text-sm text-gray-500"><span className="font-semibold">📷 클릭하여 사진 추가</span></p>
              <p className="text-xs text-gray-400">여러 장을 동시에 선택하거나 나눠서 올릴 수 있습니다.</p>
            </div>
            <input 
              type="file" 
              multiple 
